...
describe('Course CRUD tests', function () {
  ...
  it('should not be able to save an course if logged in without the "admin" role', function (done) {
  ...
  it('should not be able to update an course if signed in without the "admin" role', function (done) {
    agent.post('/api/auth/signin')
      .send(credentials)
      .expect(200)
      .end(function (signinErr, signinRes) {
  ...
  it('should be able to get a single course if not signed in', function (done) {
    ...
    // Save the course
    courseObj.save(function () {
      agent.get('/api/courses/' + courseObj._id)
        .end(function (req, res) {
          // Set assertion
  ...
  it('should return proper error for single course which doesnt exist, if not signed in', function (done) {
    ...
    agent.get('/api/courses/559e9cd815f80b4c256a8f41')
      .end(function (req, res) {
        // Set assertion
  ...
  it('should not be able to delete an course if signed in without the "admin" role', function (done) {
    agent.post('/api/auth/signin')
      .send(credentials)
      .expect(200)
      .end(function (signinErr, signinRes) {
  ...
  it('should not be able to delete an course if not signed in', function (done) {
    ...
    courseObj.save(function () {
      // Try deleting course
      agent.delete('/api/courses/' + courseObj._id)
        .expect(403)
        ...
        .end(function (signinErr, signinRes) {
          ...
          agent.post('/api/courses')
            ...
            .end(function (courseSaveErr, courseSaveRes) {
              ...
              (courseSaveRes.body.title).should.equal(course.title);
  ...
  it('should be able to get a single course if not signed in and verify the custom "isCurrentUserOwner" field is set to "false"', function (done) {
    // Create new course model instance
    var courseObj = new Course(course);
  ...
  it('should be able to get single course, that a different user created, if logged in & verify the "isCurrentUserOwner" field is set to "false"', function (done) {
    // Create temporary user creds
    var _creds = {
      usernameOrEmail: 'courseowner',
      password: 'M3@n.jsI$Aw3$0m3'
    };

    // Create user that will create the Course
    var _courseOwner = new User({
      firstName: 'Full',
      lastName: 'Name',
      displayName: 'Full Name',
      email: 'temp@test.com',
    ...
    _courseOwner.save(function (err, _user) {
      // Handle save error
      if (err) {
        return done(err);
      }

      // Sign in with the user that will create the Course
      agent.post('/api/auth/signin')
        .send(_creds)
        .expect(200)
        .end(function (signinErr, signinRes) {
          ...
          agent.post('/api/courses')
            ...
            .end(function (courseSaveErr, courseSaveRes) {
              ...
              (courseSaveRes.body.title).should.equal(course.title);
              should.exist(courseSaveRes.body.user);
              should.equal(courseSaveRes.body.user._id, userId);
                ...
                .end(function (err, res) {
                  ...
                  agent.get('/api/courses/' + courseSaveRes.body._id)
                    ...
                    .end(function (courseInfoErr, courseInfoRes) {
                      ...
                      (courseInfoRes.body.isCurrentUserOwner).should.equal(false);
